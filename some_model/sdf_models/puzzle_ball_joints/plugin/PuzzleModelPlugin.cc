...
void PuzzleModelPlugin::OnFix(ConstIntPtr &_msg)
{
  ...
  if (fix)
  {
    ignition::math::Vector3d vector3d(0, 0, 2);
    // vector3d.Z(0);
    // vector3d.Z(0);
    // vector3d.Z(2);
    ignition::math::Quaterniond q;
    ignition::math::Pose3d pose3d(vector3d, q);

    this->model->SetWorldPose(pose3d);

    this->worldJoint = this->model->GetWorld()->Physics()->CreateJoint("fixed", this->model);

    auto core = this->model->GetLink("cubie_0_0_0");
    this->worldJoint->Attach(0, core);
    this->worldJoint->Load(0, core, ignition::math::Pose3d::Zero);
  }
  else
  {
